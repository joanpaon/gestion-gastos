...
public final class CommandUsuarioListado extends Command {
    ...
    private static final String[] CAMPOS_LISTADO
            = {"usuarios.id", "usuarios.user", "perfiles.nombre"};

    // Atributo ParámetrosListado - Sesión
    private static final String PARAMETROS_LISTADO_SESION
            = "parametros-listado-usuarios";

    // Redirección Página JSP Proceso
    private static final String PAGINA_PROCESO = "usuarios/usuario-listado";
            ...
            if (!UtilesGastos.validarSesion(sesion)) {
                ...
                if (adminBLL.validarAccesoComando(getClass().getSimpleName())) {
                    ...
                    switch (usuario.getPerfilID()) {
                        case Perfil.DEVEL:
                            // BD > Lista de Usuarios
                            usuarios = usuarioDAL.obtenerUsuarios(pl);
                            break;
                        case Perfil.ADMIN:
                            // BD > Lista de Usuarios
                            usuarios = usuarioDAL.obtenerUsuarios(pl);
